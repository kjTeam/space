...
if($_POST['sure']=='yes')
  {
   $sure_id=$_POST['sure_id'];
   $query="update $pay set state='1' where id_p='$sure_id'";
   $result=$db->query($query);
   $query1="update company_result set pay_result='已缴费' where id_p='$sure_id'";
   $result1=$db->query($query1);
   if($result && $result1)
	   echo"<script languang='javascript'>alert('确认成功')</script>";
	}
     $query="select $pay.id, $pay.state, $pay.position, company_result.company, company_result.num from $pay,company_result 
  where $pay.id_p = company_result.id_p limit 0,10";
$result=$db->query($query);
$num_results=$result->num_rows;
$query="select $pay.id, $pay.state, $pay.position, company_result.company, company_result.num from $pay,company_result 
  where $pay.id_p = company_result.id_p";//查询所有的信息，这里用于计算页数和存储信息到数组
...
$query="select $pay.id, $pay.state, $pay.position, company_result.company, company_result.num from $pay,company_result 
  where $pay.id_p = company_result.id_p limit $paper_id1,$paper_id2";
$result=$db->query($query);
$num_results=$result->num_rows;
...
if($_POST['check']=='yes')
{
  ...
  $year=$_POST['year'];
  $pay ="pay".$year;
  $query="select $pay.id_p, $pay.state, $pay.position, company_result.company, company_result.num from $pay,company_result 
  where $pay.id_p = company_result.id_p and company_result.company like '%".$check."%' and $pay.state like '%".$state."%'";
  $result1=$db->query($query);
...
$FG[$i]=$row['state'];//显示当前状态 ：已处理or未处理
$SD[$i]=$row['position'];
...
<span style='padding-right:3%'>状态：<select class='form-control' data-style='btn' name='action' id='action'>
    <option value=''></option>
      <option value='0'>未审核</option>
      <option value='1'>已审核</option>
      </select></span><span >公司名称：
<input type='hidden' value='yes' name='check' id='check' >
<input type='text' name='p1' id='p1'>
<button class='btn btn-primary' style='margin:0 5% 0 2% ' type='submit'>搜索</button></form>
<input id='btnPrint' class='noprint btn btn-warning' type='button' value='打印' onclick='javascript:window.print();'
 style='font-weight:bold;text-decoration:none;cursor:pointer;important; cursor:hand'/></span></div>
...
<td>".$PD[$i]."</td>
<td>".$PS[$i]."</td>
<td >  <div class='content'> <IMG src='webpage/".$SD[$i]."' class='img1  EnlargePhoto' style='width:100px; height:50px;'></div></td>
<td>";
if ($FG[$i]==1)
echo"<span class='badge' style='background-color:green'>已处理</span>";
else
echo"<span class='badge' style='background-color:red'>未处理</span>";
echo" </td>
<td><form enctype='multipart/form-data' action='' method='post' >
<input type='hidden' value='yes' name='sure' id='sure'>
<input type='hidden' value='".$PA[$i]."' name='sure_id' id='sure_id'>
