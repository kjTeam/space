...
if($location==1) //用location取出四张表中相同的部分
{
	...
	$result1=$db->query($query1);
	$query2 = " update $sheet,join_form set $level2 = '$RE[$i]' where join_form.id_p = $sheet.id_p";
    //if($state2[$i]=='6')
		//{ 
...
if($location=='2')
{
	...
	{   
		...
		{ 
			...
			$row=$result->fetch_assoc();
		$select=$row['state']-1;
		echo"<div class='container-fluid hidden-xs' style='padding:0px 15px'>
		...
		if($row['state']=='1')  //nav==1 评审分组
		{
			...
			for($i=0;$i<$num_results;$i++)
			{
			 ...
			 echo "  </select>
				...
				<div class='container-fluid visible-xs' style='padding:0px 15px'>
				<form enctype='' action='' method='post'>
				 <tr>
				<td colspan='2' >目前进度：</td>
				<td colspan='10'>
