...
if ($_POST['send'] == 'yes') {
	...
	for($i=1;$i<= $num;$i++){
		...
		if($PF[$i]==1){
			$query2 = "update director set result = '".$PA[$i]."' where id_p = '" . $id . "' and id_f = '" . $PI[$i] . "' and form_category='2_1'";
		}else{
			$query2 = "insert into director (id_p,id_f,result,form_category) values ('" . $id . "' ,'" . $PI[$i] . "','" . $PA[$i] . "','2_1')";
		}
    ...
    for ($i = ($num+1); $i <= ($num+$num1); $i++) {
		...
		$PF[$i] = addslashes($PF[$i]);
		if($PF[$i]==1){
			$query2 = "update director set result = '".$PA[$i]."' where id_p = '" . $id . "' and id_f = '" . $PI[$i] . "' and form_category='2_2'";
		}else{
			$query2 = "insert into director (id_p,id_f,result,form_category) values ('" . $id . "' ,'" . $PI[$i] . "','" . $PA[$i] . "','2_2')";
		}
		$result2 = $db->query($query2);
	}
    if ($result) {
...
for ($i = 1; $i <= $num_results; $i++) {
    ...
    $query = "select * from director where id_p=$id and id_f=" . $row2[id] . " and form_category='2_1'";
    $result = $db->query($query);
...
for ($i = ($num_results+1); $i <= ($num_results1+$num_results); $i++) {
    ...
    $state = "state" . $i; //后面的管理员意见
    $query = "select * from director where id_p=$id and id_f=" . $row2[id] . " and form_category='2_2'";
    $result = $db->query($query);
