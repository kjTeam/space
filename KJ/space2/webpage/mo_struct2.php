...
//专项设计提交
if($_POST['mo2_zhuanxiang'] == 'yes'){
    if($_POST['flag1']=='yes')//如果之前提交过这里重复提交，将会删除之前数据。
	{
		$query="delete from mo2_zhuanxiang where id_p=$id";
...
//专项设计提交
if($_POST['mo2_chengbao'] == 'yes'){
    if($_POST['flag2']=='yes')//如果之前提交过这里重复提交，将会删除之前数据。
	{
		$query="delete from mo2_chengbao where id_p=$id";
		$result=$db->query($query);
    }
    for($i=1;$i<39;$i++)
	{
    ...
    $result_insert=insert("mo2_chengbao",39,$PA,$id);
    define('ROOT',dirname(__FILE__)); //用于上传文件 只能能上传word 这个以后要改
    $upfile1=ROOT."/mo2/mo2_chengbao//$id+file1.doc"; 
    if(file_exists($upfile1)){
        unlink($upfile1);
    }
	move_uploaded_file($_FILES['file7']['tmp_name'], $upfile1);
	$upfile2=ROOT."/mo2/mo2_chengbao//$id+file2.doc"; 
    if(file_exists($upfile2)){
        unlink($upfile2);
    }
    move_uploaded_file($_FILES['file8']['tmp_name'], $upfile2);
	$upfile3=ROOT."/mo2/mo2_chengbao//$id+file3.doc"; 
    if(file_exists($upfile3)){
        unlink($upfile3);
    }
    move_uploaded_file($_FILES['file9']['tmp_name'], $upfile3);
	$upfile4=ROOT."/mo2/mo2_chengbao//$id+file4.doc"; 
    if(file_exists($upfile4)){
        unlink($upfile4);
    }
    move_uploaded_file($_FILES['file10']['tmp_name'], $upfile4);
	$upfile5=ROOT."/mo2/mo2_chengbao//$id+file5.doc"; 
    if(file_exists($upfile5)){
        unlink($upfile5);
    }
    move_uploaded_file($_FILES['file11']['tmp_name'], $upfile5);
	$upfile6=ROOT."/mo2/mo2_chengbao//$id+file6.doc"; 
    if(file_exists($upfile6)){
        unlink($upfile6);
    }
...
// if($_POST["p32"]=='mo2') //检测是否是此表单提交
// {
// 	if($_POST['flag']=='yes')//如果之前提交过这里重复提交，将会删除之前数据。
// 	{
// 		$query="delete from mo2 where id_p=$id";
// 		$result=$db->query($query);
// 	}
// 	for($i=1;$i<32;$i++)
// 	{
...
// 	$PS[$i]=$_POST[$strr];
// 	}
// 	$result=insert2("mo2",31,$PA,$PS,$id);
// 	define('ROOT',dirname(__FILE__)); //用于上传文件 只能能上传word 这个以后要改
// 	   $upfile1=ROOT."/mo2//$id+file1.doc"; 
// 	move_uploaded_file($_FILES['file1']['tmp_name'], $upfile1);
// 	 $upfile2=ROOT."/mo2//$id+file2.doc"; 
// 	move_uploaded_file($_FILES['file2']['tmp_name'], $upfile2);
// 	$upfile3=ROOT."/mo2//$id+file3.doc"; 
// 	move_uploaded_file($_FILES['file3']['tmp_name'], $upfile3);
// 	$upfile4=ROOT."/mo2//$id+file4.doc"; 
// 	move_uploaded_file($_FILES['file4']['tmp_name'], $upfile4);
// 	$upfile5=ROOT."/mo2//$id+file5.doc"; 
// 	move_uploaded_file($_FILES['file5']['tmp_name'], $upfile5);
// 	$upfile6=ROOT."/mo2//$id+file6.doc"; 
// 	move_uploaded_file($_FILES['file6']['tmp_name'], $upfile6);
// 	$upfile7=ROOT."/mo2//$id+file7.doc"; 
// 	move_uploaded_file($_FILES['file7']['tmp_name'], $upfile7);
// 	$upfile8=ROOT."/mo2//$id+file6.doc"; 
// 	move_uploaded_file($_FILES['file8']['tmp_name'], $upfile8);
// 	$upfile9=ROOT."/mo2//$id+file6.doc"; 
// 	move_uploaded_file($_FILES['file9']['tmp_name'], $upfile9);
// 	$upfile10=ROOT."/mo2//$id+file6.doc"; 
// 	move_uploaded_file($_FILES['file10']['tmp_name'], $upfile9);
// 	$upfile11=ROOT."/mo2//$id+file6.doc"; 
// 	move_uploaded_file($_FILES['file11']['tmp_name'], $upfile9);
// 	$upfile12=ROOT."/mo2//$id+file6.doc"; 
...
<p style='color:#ccc;margin:5px;'>中国钢结构协会空间结构分会膜结构专项设计企业等级会员复审表</p>";
//获取专项设计审核状态
$db=create_database();
$query="select * from mo2_zhuanxiang where id_p=$id ";
$result=$db->query($query);
...
echo"
   <form enctype='multipart/form-data' action='' method='post' id='mo_zhuanxiang'>
        <table class='table table-bordered text-center table-responsive' name='design' >
            <tbody>
			<h3 class='text-center'>中国钢结构协会空间结构分会膜结构专项设计企业等级会员复审表</h3>
	        <br/>
            <tr>
        ...
        <input type='hidden' value='yes' name='mo2_zhuanxiang'>
    ...
    echo"<div style='border:solid 1px #ccc; padding:5px 10px 50px 10px;margin:20px 0 10px 0'>
        <p style='color:#ccc;margin:5px;'>中国钢结构协会空间结构分会膜结构工程承包企业等级会员复审表</p>";
    //获取专项设计审核状态
$db=create_database();
$query="select * from mo2_chengbao where id_p=$id ";
$result=$db->query($query);
$num_results=$result->num_rows;
$flag2='';
if($num_results)	
{
    ...
    echo"<form enctype='multipart/form-data' action='' method='post' id='mo_chengbao'>
        <table class='table table-bordered text-center table-responsive' name='chengbao' >
            <tbody>
			<h3 class='text-center'>中国钢结构协会空间结构分会膜结构工程承包企业等级会员复审表</h3>
	        <br/>
            <tr>
               <td colspan='2'>单位名称</td>
...
<input type='hidden' value='yes' name='mo2_chengbao'>
...
echo"<h5 class='text-center'><strong>中国钢结构协会空间结构分会膜结构专项设计企业等级会员复审表</strong></h5>
...
if($num_results)	
{
  ...
  echo"<h5 class='text-center'><strong>中国钢结构协会空间结构分会膜结构工程承包企业等级会员复审表</strong></h5>
