...
{
  $edit_id=$_POST['edit_id'];
  $query="select * from pm1 where id='$edit_id'";
  $result1=$db->query($query);
  $row=$result1->fetch_assoc(); 
echo "
<div class='container-fluid'>
<div class='col-xs-6' style='margin-bottom:10px;font-size:16px'>
<a href=''>/  企业膜经理  /  </a>".$row['name']."资料
</div>
<form class='form-signin' action='' method='post'  style='margin-top:10px;'>
<table class='table table-bordered table-responsive text-center'>
        <tr>
            <th colspan='12' style='text-align:center;'><lead>企业膜经理资料</lead></th>
        </tr>
        <tr>
            <td colspan='2'> 姓名</td>
            <td colspan='10'> <input type='text' id='uid' name='uid' class='form-control' value=".$row['c1']." ></td>
        </tr>
		<tr>
            <td colspan='2'> 所在单位</td>
            <td colspan='10'> <input type='password' id='psw1'  name='psw1' class='form-control' value=".$row['c8']."></td>
        </tr>
		<tr>
            <td colspan='2'> 等级</td>
            <td colspan='10'> <input id='psw2' name='psw2' class='form-control' value=".$row['result']."></td>
        </tr>
		<tr>
            <td colspan='2'> 证书编号</td>
            <td colspan='10'> <input type='text' id='name' name='name' class='form-control' value=".$row['number']."></td>
        </tr>
		<tr>
		<td colspan='2'> </td>
		<td  colspan='12'>
		<select class='input form-control'>
            <option value='1'>未通过培训</option>
			 <option value='1'>已通过培训</option>
</select></td>
        </tr>
...
if($_POST['del']=='yes')
  {
   $del_id=$_POST['del_id'];
   $query="delete from pm1 where id='$del_id'";
   $result=$db->query($query);
   if($result)
	   echo"<script languang='javascript'>alert('删除成功')</script>";
	}
$query="select * from pm1 limit 0,10";//查询前10行的信息，用于显示第一页
$result=$db->query($query);
$num_results=$result->num_rows;
$query="select * from pm1 ";//查询所有的信息，这里用于计算页数和存储信息到数组
$result1=$db->query($query);
...
$query="select *from pm1 limit $paper_id1,$paper_id2";
$result=$db->query($query);
$num_results=$result->num_rows;
...
{
  $check=$_POST['p1'];
	 $query="select * from pm1 where uid like '%".$check."%' or name='%".$check."%'";
     $result1=$db->query($query);
