...
$con=connect_mysql();
$result=$con->query("select id,token_exptime from user where mail_activation_statu='0' and `token`='$token'");
...
if($row){
    if($nowtime>$row['token_exptime']){ //24hour
        ...
        $res=$con->query("update user set mail_activation_statu='1' where id={$row['id']}");
